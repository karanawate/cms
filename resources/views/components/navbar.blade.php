    ...
    <div class="relative flex items-center justify-between h-16">
      ...
      <div class="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
        <div class="flex-shrink-0 flex items-center">
            <img src="{{ url(@$meta['logo']) }}" alt="" class="w-8 h-8 rounded-full">
        ...
        <div class="hidden sm:block sm:ml-6">
          <div class="flex space-x-4">
            ...
            <form action="">
            <input type="text" name="first_name" id="first_name" placeholder="Search" autocomplete="given-name" class="rounded-lg px-3  w-full py-1 transition duration-300 border border-gray-300 rounded focus:border-transparent focus:outline-none focus:ring-2 focus:ring-blue-400">
            </form>
          </div>
        </div>
      </div>
      <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
        ...
        <div class="ml-3 relative" x-data="{ open: false }" @click.away="open = false">
          <div>
            <button @click="open = !open"  class="flex items-center text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white" id="user-menu" aria-haspopup="true">
              <span class="sr-only">Open user menu</span>
                    <?php  $name = Auth::user()->name; ?>
              <img class="h-8 w-8 rounded-full" src="https://ui-avatars.com/api/?bold=true&background=random&name={{ $name }}%22" alt="">
              <div class="ml-2 ">
                <div>{{ Auth::user()->name }}</div>
                <div class="-mt-1 text-gray-500 text-xs">{{ str_limit(Auth()->user()->email , 11) }}</div>
              </div>
              <span class="ml-2 ">
          ...
          <div x-show="open" class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
            <a href="profile" class="flex px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem">
            ...
            <span class="ml-3 text-sm mr-2">Your Profile</span>
            </a>
            ...
            <a href="#" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="flex px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem">
